<template>
 ...
 <div class="grid grid-cols-4 gap-4 mt-10">
    <div v-for="(commande, index) in commandes" :key="index" class="w-full max-w-sm px-4 py-3 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
        <div class="flex items-center justify-between">
            <span class="text-sm font-light text-gray-800 dark:text-gray-400">Courses</span>
            <span class="px-3 py-1 text-xs text-blue-800 uppercase bg-blue-200 rounded-full dark:bg-blue-300 dark:text-blue-900">N° {{ index+1 }}</span>
        </div>

        <div>

            <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Price: $ {{ commande.price }}</p>
        </div>

        <div>
            <div class="flex items-center mt-2 text-gray-700 dark:text-gray-200">
                <span>Date</span>
                <p class="mx-2 text-sm text-blue-600 cursor-pointer dark:text-blue-400 hover:underline">{{ commande.date }} .</p>
                <router-link  :to="{
                            name: 'Commande',
                            params: {
                                id: commande.id,
                            },
                        }" class="mx-2 text-blue-600 cursor-pointer dark:text-blue-400 hover:underline"> More</router-link>
            </div>
<!-- 
    ...
    mounted() {
        ...
        axios.get(`https://api-moshop.molengeek.pro/api/v1/orders`, {
            ...
            .then(response => {
                ...
                localStorage.setItem('commandes', JSON.stringify(this.commandes.id))
            })
            .catch(error => {
                console.log(error)
            })
